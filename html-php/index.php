...
<body onload="ablakok()" onresize="ablakok()">
    ...
    <!--           Jelszó csere              -->
            ...
            <div id="uzenet"></div>
    <!--           Profil                    -->
            <div id="profil" style="overflow-x: auto;">
                <table>
                <tr><th>Vezeték Név</th><th>Kereszt név</th><th>Email cím</th><th>Születési dátum</th><th>Neme</th><th>Magasság</th></tr>
                    <?php
                        $sqlmondat="SELECT vnev, knev, email, szuldatum, neme, magassag FROM felhasznalo WHERE id=".$_SESSION['id'];
    ...
    <!--           Adminpanel felhasználók   -->
            <div id="users" style="overflow-x: auto;">
                <table>
                    <tr><th></th><th>ID</th><th>Vezeték Név</th><th>Kereszt név</th><th>Email cím</th><th>Születési dátum</th><th>Neme</th><th>Magasság</th><th>jog</th></tr>
                    <?php
                        $sqlmondat="SELECT * FROM felhasznalo";
                        $eredmeny=mysqli_query($kapcsolat, $sqlmondat);
                        while($adatsor=mysqli_fetch_array($eredmeny)){
                            echo "<tr><td><input type=\"checkbox\" id=\"checkItem\" name=\"check[]\" value=\"".$adatsor['id']."\"></td><td>".$adatsor['id']."</td><td>".$adatsor['vnev']."</td><td>".$adatsor['knev']."</td><td>".$adatsor['email']."</td><td>".$adatsor['szuldatum']."</td><td>".$adatsor['neme']."</td><td>".$adatsor['magassag']."</td><td>".$adatsor['jog']."</td></tr>";
                        }
                    ?>
                </table>
            </div>
